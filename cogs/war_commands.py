...
class WarCommands(commands.Cog):
    ...
    async def migrate_war(self, ctx):
        ...
        Usage: !migratewar
        """
        # Access your MongoDB collection (Adjust 'db_collection' or database connection reference as per your bot structure)
        db_collection = getattr(self.bot, "db_collection", None)
        
        if db_collection is None:
            # Fallback if collection is stored inside a cog or custom bot property
            await ctx.send("❌ Could not connect to MongoDB collection instance on bot.")
            return
