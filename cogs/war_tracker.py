...
class WarTracker(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # Connect DB and start background loops
        self.init_mongodb()
    ...
    async def db_add_clan(self, tag, name, channel_id, guild_id):
        await self.clans_collection.update_one(
            ...
            {"$set": {"clan_name": name, "channel_id": channel_id}, "$setOnInsert": {"last_match_id": None, "last_phase": None}},
            upsert=True
        )

    async def db_update_war_state(self, tag, guild_id, current_phase, opponent_tag):
    ...
    @app_commands.command(name="checkwar", description="Instantly check live status and generate war summary.")
    ...
    async def checkwar_command(self, interaction: discord.Interaction, clan_tag: str):
        ...
        try:
            ...
            await generate_and_store_war_conflict(clan_tag, interaction.guild_id, self.conflicts_collection)
    ...
    @app_commands.command(name="addclan", description="Register a new clan tag for tracking.")
    async def addclan(self, interaction: discord.Interaction, clan_tag: str):
        ...
        if formatted_tag in guild_clans:
            await interaction.followup.send(f"⚠️ `{formatted_tag}` is already tracked.")
            return
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(BASE_GATEWAY, params=params) as response:
                if response.status != 200:
                    await interaction.followup.send("❌ Registration rejected. Please check tag.")
                    return
        ...
        await interaction.followup.send(f"✅ Registered **{clan_name}** (`{formatted_tag}`).")

    @app_commands.command(name="removeclan", description="Stop tracking a clan tag.")
    @app_commands.autocomplete(clan_tag=clan_autocomplete)
    async def removeclan(self, interaction: discord.Interaction, clan_tag: str):
        ...
        if formatted_tag not in guild_clans:
            await interaction.response.send_message("❌ That clan is not tracked.", ephemeral=True)
            return

        name = guild_clans[formatted_tag]["clan_name"]
        await self.db_remove_clan(formatted_tag, interaction.guild_id)
        await interaction.response.send_message(f"🗑️ Removed **{name}** (`{formatted_tag}`).", ephemeral=True)

    @app_commands.command(name="listclans", description="List all tracked clans in this server.")
    async def listclans(self, interaction: discord.Interaction):
        if not interaction.guild_id: return
        guild_clans = await self.db_get_guild_clans(interaction.guild_id)
        if not guild_clans:
            await interaction.response.send_message("📭 No tracked clans.", ephemeral=True)
            return

        embed = discord.Embed(title="📋 Tracked Clans", color=0x336869)
        for tag, details in guild_clans.items():
            embed.add_field(name=f"{details['clan_name']} ({tag})", value=f"Channel: <#{details['channel_id']}>", inline=False)
